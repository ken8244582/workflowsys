...
export default function FlowsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/flow-data.json')
      .then((res) => res.json())
      .then((raw: FlowItem[]) => {
        // Only show items with L4 process
  ...
  const filtered = useMemo(() => {
    ...
    if (l1Filter && l1Filter !== 'all') {
      result = result.filter((d) => d.l1Domain === l1Filter);
    }
    ...
    if (itFilter && itFilter !== 'all') {
      result = result.filter((d) => d.itCoverage === itFilter);
    }
    if (search.trim()) {
  ...
  useEffect(() => {
    setPage(1);
  }, [l1Filter, categoryFilter, formatFilter, itFilter, search]);
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">流程清单筛选</CardTitle>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            ...
            <Select value={l1Filter} onValueChange={setL1Filter}>
              <SelectTrigger className="text-sm">
                <SelectValue placeholder="L1 业务域" />
              </SelectTrigger>
              <SelectContent>
                ...
                {pageData.map((item, idx) => (
                  <tr key={`${item.id}-${idx}`} className="border-b last:border-0 hover:bg-muted/30">
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-xs">
                      {item.format && (
                        <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${FORMAT_COLORS[item.format] || 'bg-gray-200 text-gray-700'}`}>
                          {item.format}
                        </span>
                      )}
                    </td>
                    <td className="whitespace-nowrap px-3 py-2 text-xs">
                      {item.category && (
                        <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${CATEGORY_COLORS[item.category] || 'bg-gray-200 text-gray-700'}`}>
                          {item.category}
                        </span>
                      )}
                    </td>
                    <td className="whitespace-nowrap px-3 py-2 text-xs">
                      {item.itCoverage && (
                        <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${IT_COLORS[item.itCoverage] || 'bg-gray-200 text-gray-700'}`}>
                          {item.itCoverage}
                        </span>
                      )}
                    </td>
