...
export default function FunctionalRevisionPage() {
  ...
  useEffect(() => {
    fetch('/revision-plan.json')
      .then((res) => res.json())
      .then((raw: RevisionItem[]) => { setData(raw); setLoading(false); })
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== 'all') {
      if (statusFilter === '未完成') result = result.filter((d) => !d.完成情况);
      else result = result.filter((d) => d.完成情况 === statusFilter);
    }
    return result;
  ...
  const typeDistribution = useMemo(() => {
    ...
    data.forEach((d) => { if (d.修订类型) map.set(d.修订类型, (map.get(d.修订类型) || 0) + 1); });
  ...
  const statusDistribution = useMemo(() => {
    ...
    return [{ name: '已完成', value: completed }, { name: '未完成', value: incomplete }];
  }, [data]);
  ...
  const deptDistribution = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([name, counts]) => ({ name, ...counts })).sort((a, b) => b.total - a.total);
  }, [data]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-semibold text-[#1e3a5f]">职能流程修订记录</h2>
      </div>

      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Card className="border-[#1e3a5f]/20 bg-[#1e3a5f]/5"><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">修订计划总数</p><p className="mt-1 text-2xl font-bold tabular-nums text-[#1e3a5f]">{stats.total}</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">已完成</p><p className="mt-1 text-2xl font-bold tabular-nums">{stats.completed}</p><p className="mt-0.5 text-[11px] text-muted-foreground">完成率 {stats.completionRate}%</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">修订类</p><p className="mt-1 text-2xl font-bold tabular-nums">{stats.revision}</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs font-medium text-muted-foreground">新增类</p><p className="mt-1 text-2xl font-bold tabular-nums">{stats.newAdd}</p></CardContent></Card>
      </div>

      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">修订类型分布</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={typeDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }: { name: string; percent: number }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                  {typeDistribution.map((_, index) => (<Cell key={`type-${index}`} fill={['#1e3a5f', '#f59e0b'][index] || CHART_COLORS[index]} />))}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">完成情况</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={statusDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} label={({ name, percent }: { name: string; percent: number }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                  {statusDistribution.map((_, index) => (<Cell key={`status-${index}`} fill={['#10b981', '#94a3b8'][index] || CHART_COLORS[index]} />))}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm">各业务域修订计划</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">共 <span className="font-semibold tabular-nums text-foreground">{filtered.length}</span> 条</span>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-center gap-3">
            <Select value={deptFilter} onValueChange={setDeptFilter}>
              <SelectTrigger className="w-[180px] text-sm"><SelectValue placeholder="所属业务域" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={typeFilter} onValueChange={setTypeFilter}>
              <SelectTrigger className="w-[120px] text-sm"><SelectValue placeholder="修订类型" /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-[120px] text-sm"><SelectValue placeholder="完成情况" /></SelectTrigger>
              <SelectContent>
                ...
                {paged.map((item, idx) => (
                  <tr key={idx} className="border-b last:border-0 hover:bg-muted/30">
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-xs">
                      {item.修订类型 && <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${TYPE_STYLES[item.修订类型] || 'bg-gray-200 text-gray-700'}`}>{item.修订类型}</span>}
                    </td>
                    <td className="whitespace-nowrap px-3 py-2 text-xs">
                      <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${item.完成情况 === '已完成' ? 'bg-[#10b981] text-white' : 'bg-[#94a3b8] text-white'}`}>{item.完成情况 || '未完成'}</span>
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && <tr><td colSpan={8} className="px-3 py-12 text-center text-muted-foreground">无匹配结果</td></tr>}
              </tbody>
            </table>
          </div>
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-2">
              <p className="text-xs text-muted-foreground">第 {(page - 1) * pageSize + 1}-{Math.min(page * pageSize, filtered.length)} 条，共 {filtered.length} 条</p>
