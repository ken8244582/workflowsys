...
function SectionTitle(
    ...
    return (
        <div className="flex items-center justify-between gap-3 pb-1">
            ...
            {links && links.length > 0 && <div className="flex items-center gap-4">
                {links.map((link, i) => <Link
                    key={i}
                    href={link.href}
                    className="text-sm text-[#1e3a5f] hover:underline">{link.text}→</Link>)}
            </div>}
        </div>
    );
}
...
function StatCard(
    ...
    return (
        <Card
            ...
            {accent && <div
                className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-[#1e3a5f] to-[#3b82f6]" />}
            <CardContent className="flex items-start gap-3 p-4">
                <div
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#1e3a5f]/10 text-[#1e3a5f]">
                    {icon}
                </div>
                ...
                <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                        <Pie
                            ...
                            fontSize={10}>
                            {data.map(
                                (_, i) => <Cell key={i} fill={["#1e3a5f", "#3b82f6", "#94a3b8"][i] || "#64748b"} />
                            )}
                        </Pie>
                        <Tooltip />
                    </PieChart>
                ...
                <ResponsiveContainer width="100%" height={180}>
                    <PieChart>
                        <Pie
                            ...
                            fontSize={10}>
                            {data.map(
                                (_, i) => <Cell key={i} fill={["#10b981", "#f59e0b", "#94a3b8"][i] || "#64748b"} />
                            )}
                        </Pie>
                        <Tooltip />
                    </PieChart>
                ...
                <ResponsiveContainer width="100%" height={240}>
                    <PieChart>
                        <Pie
                            ...
                            fontSize={11}>
                            {data.map((d, i) => <Cell key={i} fill={colorMap[d.status] || CHART_COLORS[i]} />)}
                        </Pie>
                        <Tooltip />
                        <text
                            x="50%"
                        ...
                        <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="middle"
                            className="fill-muted-foreground text-[10px]">任务总数
                                                                    </text>
                    </PieChart>
                </ResponsiveContainer>
            </CardContent>
                ...
                <table className="w-full text-xs">
                    <thead>
                        <tr className="border-b text-left">
                            <th
                                className="whitespace-nowrap px-2 py-1.5 font-medium text-muted-foreground">业务域</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">L4流程</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">流程</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">办法</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">其它</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">集团模板</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">旧格式</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">IT覆盖</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map(
                            row => <tr key={row.name} className="border-b last:border-0 hover:bg-muted/50">
                                <td className="whitespace-nowrap px-2 py-1.5 font-medium">{row.name}</td>
                                <td
                                    className="whitespace-nowrap px-2 py-1.5 text-right tabular-nums font-semibold">{row.l4Count}</td>
...
function E2EProgressChart(
    ...
    const chartData = [...data].sort((a, b) => b.currentProgress - a.currentProgress).map(d => ({
                ...
                <ResponsiveContainer width="100%" height={240}>
                    <PieChart>
                        ...
                        <text
                            ...
                            textAnchor="middle"
                            dominantBaseline="middle"
                            className="fill-muted-foreground text-[10px]">计划总数
                                                                    </text>
                    </PieChart>
                </ResponsiveContainer>
            </CardContent>
            ...
            <CardContent className="overflow-auto">
                <table className="w-full text-xs">
                    <thead>
                        <tr className="border-b text-left">
                            <th
                                className="whitespace-nowrap px-2 py-1.5 font-medium text-muted-foreground">端到端流程</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 font-medium text-muted-foreground">负责部门</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 text-right font-medium text-muted-foreground">当前完成</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 font-medium text-muted-foreground">状态</th>
                            <th
                                className="whitespace-nowrap px-2 py-1.5 font-medium text-muted-foreground">进度条</th>
                        </tr>
                    </thead>
                    <tbody>
                        {sortedData.map(row => {
                            ...
                            return (
                                <tr key={row.id} className="border-b last:border-0 hover:bg-muted/50">
                                    ...
                                    <td
                                        className="whitespace-nowrap px-2 py-1.5 text-right tabular-nums font-semibold">{row.currentProgress}%</td>
...
export default function DashboardPage() {
        ...
        return (
            <div className="flex h-[60vh] items-center justify-center">
                <div className="text-center">
                    <div
                        className="mx-auto h-8 w-8 animate-spin rounded-full border-2 border-[#1e3a5f] border-t-transparent" />
    ...
    const groupTemplateRate = stats.l4ProcessCount > 0 ? (stats.groupTemplateCount / stats.l4ProcessCount * 100).toFixed(1) : "0";
    const coverageRate = stats.l4ProcessCount > 0 ? (stats.itYesCount / stats.l4ProcessCount * 100).toFixed(1) : "0";
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
                <StatCard
                    title="L1 业务域"
                    value={stats.l1DomainCount}
                    accent
                    icon={<Building2 className="h-5 w-5" />} />
                <StatCard
                    title="L4 职能流程"
                    value={stats.l4ProcessCount}
                    subtitle={`总条目 ${stats.totalRows}`}
                    accent
                    icon={<FileText className="h-5 w-5" />} />
                <StatCard
                    title="集团模板占比"
                    value={`${groupTemplateRate}%`}
                    subtitle={`${stats.groupTemplateCount} / ${stats.l4ProcessCount}`}
                    icon={<Shield className="h-5 w-5" />} />
                <StatCard
                    title="IT 覆盖率"
                    value={`${coverageRate}%`}
                    subtitle={`已覆盖 ${stats.itYesCount}`}
                    icon={<Wifi className="h-5 w-5" />} />
                <StatCard
                    title="修订计划数"
                    value={revision?.totalPlans ?? "--"}
                    subtitle={`已下发 ${revision?.publishedPlans ?? 0}`}
                    icon={<ClipboardList className="h-5 w-5" />} />
                <StatCard
                    title="修订完成率"
                    value={revision ? `${revision.completionRate}%` : "--"}
                    subtitle={`完成 ${revision?.totalCompleted ?? 0} / ${revision?.totalTasks ?? 0}`}
            ...
            {}
            {revision && revision.monthlyTrend.length > 0 && <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
                ...
                <StatCard
                    title="已完成数"
                    value={completedE2E}
                    subtitle={`共 ${e2eTotal} 条`}
                    icon={<CheckCircle className="h-5 w-5" />} />
                <StatCard
                    title="梳理计划数"
                    value={e2e?.planTotal ?? "--"}
                    subtitle={`进行中 ${e2e?.planInProgress ?? 0}`}
                    icon={<ClipboardList className="h-5 w-5" />} />
                <StatCard
                    title="梳理完成率"
                    value={e2e ? `${e2e.planCompletionRate}%` : "--"}
                    subtitle={`完成 ${e2e?.planCompleted ?? 0} / ${e2e?.planTotal ?? 0}`}
                    icon={<Target className="h-5 w-5" />} />
                <StatCard
                    title="平均计划进度"
                    value={e2e ? `${e2e.avgPlanProgress}%` : "--"}
                    subtitle={`实际进度 ${e2e?.avgActualProgress ?? 0}%`}
            ...
            {}
            <div
                className="border-t pt-4 flex items-center justify-between text-xs text-muted-foreground">
