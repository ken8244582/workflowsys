...
使用统一 `<PaginationBar>` 组件，**表格上下各放置一个**，无 Card 包裹：
...
- 表格去内边距 `p-0`，表格自身处理间距
- 行内操作按钮无边框 `variant="ghost"`，仅图标无文字，**必须添加 `title` 属性**提供悬浮提示
- 新增/主操作用实底深靛蓝，导出/次要操作用 outline 描边
- 危险操作统一红色系（边框 `border-red-200`、文字 `text-red-600`、行内 `text-red-500`）

## 交互与状态
- 会话超时：Cookie为Session模式（浏览器关闭失效）+ Bearer Token 双重认证，JWT 1小时过期后自动跳转登录页
